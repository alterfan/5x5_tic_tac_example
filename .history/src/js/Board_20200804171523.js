...
export default class Board extends Phaser.GameObjects.Container {
	...
	makeMove(cell) {
		if (this.state.whoseTurn == 'x') {
			...
			this.AI.makeMove(this.matrix)
			this.draw(this.AI.getBetterPosition())
			...
			for (var col = 0; col < this.sideCellsCount; col++) {
				if (Helper.getChar(this.matrix, row, col) != '0') {
					...
					if (row <= this.sideCellsCount - rule) {
						...
						if (col >= rule - 1) {
							winning = 1
							while (Helper.getChar(this.matrix, row, col) == Helper.getChar(this.matrix, row + winning, col - winning)) {
								winning += 1
							}
							if (winning >= rule) {

			...
			if (getChar(board, row, col) == symbol) {
				...
				if (row <= Defaults.ROWS_COLS - rule) {
					...
					if (col >= rule - 1) {
						...
						if (winning == rule) {
							if (
								getChar(board, row + winning, col - winning) == '0' &&
								getChar(board, row - 1, col + 1) == '0'
							) {
								opened += 1
							} else {
								closed += 1
...
export { Dom, Transform, switchTurn, getStateResult as get_result, getChar as get_symbol, getBoardCopy as get_board_copy, checkRow as check_row, isBetter as is_better, aiMakeMove as bot_move }

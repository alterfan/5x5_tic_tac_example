...
export default class Board extends Phaser.GameObjects.Container {
	...
	makeMove(cell) {
		...
		if (this.state.whoseTurn == 'x' && cell != undefined) {
			...
			this.draw(this.AI.getTargetPos())
		}
		let count = 0
	...
	draw(pos) {
		setTimeout(() => {
			...
			if (this._matrix[pos] == '0') {
				...
				this.scene.cell_matrix[col][row].char = this.scene.add.image(x, y, this._matrix[pos]).setAlpha(0)
				this.tweens.add(
					{
						targets: this.cell_matrix[col][row].char,
						alpha: 1,
						duration: 200,
						ease: 'Power2',
					},
